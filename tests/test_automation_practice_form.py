...
class TestFormPage:
    ...
    def test_form(self, driver, gender, hobbies, state, city, subjects):
        ...
        form_page.open()
        input_person = form_page.fill_form_fields(student_gender=gender, student_hobbies=hobbies, state=state,
        ...
        assert gender == output_table[2]
        assert input_person.mobile[:-3] == output_table[3]
        assert subjects == output_table[5]
        assert input_person.current_address == output_table[8]
        assert output_table[9] == state + " " + city
